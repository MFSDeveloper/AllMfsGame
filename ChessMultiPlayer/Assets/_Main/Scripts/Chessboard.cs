...
namespace Chessticle
{
    ...
    public class Chessboard : MonoBehaviour
    {
        ...
        void UnmakeMove(Move move)
        {
            ...
            bool isCastling = piece == Piece.King && (offset == 2 || offset == -2);
            if (isCastling)
            {
                bool isKingSide = offset == 2;
                int rookStartIdx = move.StartIdx + (isKingSide ? 3 : -4);
                int rookTargetIdx = move.TargetIdx + (isKingSide ? -1 : 1);
                m_Board[rookStartIdx] = PieceToBoardValue(Piece.Rook, color);
                m_Board[rookTargetIdx] = k_EmptySquare;
            }
        }
